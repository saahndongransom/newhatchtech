...
def newsletter(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print(f"bcc: {receivers}")
            email_from = f"nblog <{request.user.email}>" if request.user.email else "nblog"
            mail = EmailMessage(subject, strip_tags(email_message), email_from, bcc=receivers)
            #mail = EmailMessage(subject, strip_tags(email_message), f"nblog <{request.user.email}>", bcc=receivers)
            #mail = EmailMessage(subject, email_message, f"nblog <{request.user.email}>", bcc=receivers)
            mail.content_subtype = 'html'
            if subject is None or email_message is None or request.user.email is None:
...
def post_detail(request, pk):
    ...
    if request.method == 'POST':
        ...
        form = CommentForm()
    return render(request, 'nblog/post_detail.html', {'post': post, 'comments': comments, 'form': form})
